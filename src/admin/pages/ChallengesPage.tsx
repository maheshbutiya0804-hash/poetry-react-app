          ...
          <section className="relative rounded-[30px] border border-[rgba(57,47,39,0.12)] bg-[#fcfaf7] p-6 shadow-[0_18px_40px_rgba(47,37,28,0.06)] max-[720px]:p-[18px]" aria-labelledby="challenges-table-title">
            ...
            {loading ? (
              ...
              <div className="overflow-x-auto rounded-[24px] border border-[rgba(57,47,39,0.12)] bg-[rgba(252,250,247,0.88)] max-[720px]:border-0 max-[720px]:bg-transparent">
                <div className="min-w-[900px] max-[720px]:hidden">
                  <table className="w-full border-collapse">
                    <thead><tr className="bg-[rgba(241,235,226,0.45)]">
                      {['Challenge','Month','Status','Reminders','Updated','Actions'].map(label => <th key={label} className="border-b border-[rgba(57,47,39,0.12)] px-5 py-[18px] text-left text-[0.74rem] font-bold uppercase tracking-[0.16em] text-[#776d62]">{label}</th>)}
                    </tr></thead>
                    <tbody>{data.challenges.map((challenge, index) => <tr key={challenge.id} className="transition hover:bg-[rgba(23,57,47,0.028)]">
                      <td className={`${index ? 'border-t border-[rgba(57,47,39,0.08)] ' : ''}px-5 py-[18px] align-middle`}><div className="flex items-center gap-3.5">{challenge.imageUrl ? <img src={challenge.imageUrl} alt="" className="h-[64px] w-[82px] rounded-[16px] border border-[rgba(57,47,39,0.1)] object-cover"/> : <div className="h-[64px] w-[82px] rounded-[16px] border border-[rgba(57,47,39,0.1)] bg-[linear-gradient(135deg,#ece2d5,#d9cdbf)]"/>}<div><strong className="block">{challenge.title}</strong><span className="mt-1 block text-[0.84rem] text-[#776d62]">Monthly subscriber challenge</span></div></div></td>
                      ...
                      <td className={`${index ? 'border-t border-[rgba(57,47,39,0.08)] ' : ''}px-5 py-[18px] align-middle`}><div className="inline-flex items-center gap-2"><button type="button" title={challenge.status === 'PUBLISHED' ? 'Unpublish challenge' : 'Publish challenge'} onClick={() => toggle(challenge.id, challenge.status)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[rgba(57,47,39,0.12)] bg-[#f9f5ef] text-[#17392f] transition hover:-translate-y-px hover:bg-[#f6f0e8]">{challenge.status === 'PUBLISHED' ? '◌' : '✓'}</button><button type="button" title="Delete challenge" onClick={() => setDeleteId(challenge.id)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[rgba(57,47,39,0.12)] bg-[#f9f5ef] text-[#776d62] transition hover:-translate-y-px hover:bg-[#f6f0e8]">⌫</button></div></td>
                    </tr>)}</tbody>
                  </table>
                </div>

                <div className="hidden gap-3 max-[720px]:grid">{data.challenges.map(challenge => <article key={challenge.id} className="rounded-[22px] border border-[rgba(57,47,39,0.12)] bg-[#f9f5ef] p-4 shadow-[0_10px_22px_rgba(47,37,28,0.04)]"><div className="flex items-start gap-3">{challenge.imageUrl ? <img src={challenge.imageUrl} alt="" className="h-[72px] w-[92px] rounded-[16px] object-cover"/> : <div className="h-[72px] w-[92px] rounded-[16px] bg-[linear-gradient(135deg,#ece2d5,#d9cdbf)]"/>}<div className="min-w-0"><strong className="block truncate">{challenge.title}</strong><span className="mt-1 block text-sm text-[#776d62]">{monthFmt(challenge.challengeMonth)}</span></div></div><div className="mt-4 grid grid-cols-2 gap-3"><div><span className="block text-[0.7rem] uppercase tracking-[0.14em] text-[#776d62]">Status</span><span className={`mt-1 inline-flex min-h-[34px] items-center rounded-full border px-3 text-[0.84rem] font-bold ${statusClass(challenge.status)}`}>{challenge.status}</span></div><div><span className="block text-[0.7rem] uppercase tracking-[0.14em] text-[#776d62]">Reminders</span><span className="mt-2 block text-sm">{challenge.reminders.length}</span></div></div><div className="mt-4 flex gap-2"><button type="button" onClick={() => toggle(challenge.id, challenge.status)} className="min-h-10 rounded-full border border-[rgba(57,47,39,0.12)] bg-[#fcfaf7] px-4 text-sm font-semibold text-[#17392f]">{challenge.status === 'PUBLISHED' ? 'Unpublish' : 'Publish'}</button><button type="button" onClick={() => setDeleteId(challenge.id)} className="min-h-10 rounded-full border border-[rgba(125,74,74,0.18)] bg-[rgba(125,74,74,0.06)] px-4 text-sm font-semibold text-[#7a4a4a]">Delete</button></div></article>)}</div>
              </div>
            )}
