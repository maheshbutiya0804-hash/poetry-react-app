...
import { adminCreateDesignedCard, adminGetCard, adminUpdateDesignedCard, adminUpdateBulkCard, adminGetCollections, type AdminCollection } from '../../services/api'
...
export function CreateCardPage(){
  ...
  useEffect(()=>{adminGetCollections().then(setCols).catch(e=>setMsg(e.message))},[])
  useEffect(()=>{
    if(!cardId) return
    setLoadingCard(true)
    adminGetCard(cardId).then(card=>{
      ...
      setIsBulkUploaded(Boolean(card.isBulkUploaded))
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      if(isEdit && cardId && isBulkUploaded){
        await adminUpdateBulkCard(cardId,{collectionId,title,published:status==='PUBLISHED',featured})
      }else{
        ...
        if(isEdit && cardId) await adminUpdateDesignedCard(cardId,fd); else await adminCreateDesignedCard(fd)
      }
      nav('/admin/cards')
