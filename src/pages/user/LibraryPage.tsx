...
export function LibraryPage(){
  ...
  return <main className="min-h-[calc(100vh-88px)] bg-[#f8f3eb] px-8 py-12 text-[#2b2621] md:px-16 lg:px-24">
    <section className="mx-auto max-w-[1360px]"><h1 className="font-serif text-[clamp(3rem,5vw,4.7rem)] font-semibold leading-none tracking-[-.04em]">Your Library</h1><p className="mt-5 max-w-[780px] text-[1.08rem] leading-8 text-[#756b61]">Cards you've saved to return to, share, or send – alongside poem requests still<br className="hidden md:block"/> being crafted and those ready to revisit.</p></section>

    {active===false?<section className="mx-auto mt-10 grid min-h-[430px] max-w-[1360px] place-items-center rounded-[30px] border border-[#e2d8cb] bg-[#fcfaf7] px-8 text-center shadow-[0_14px_34px_rgba(45,34,24,.05)]"><div><div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-[#e7e7e1] text-[2.2rem] text-[#17392f]">▢</div><h2 className="mt-7 font-serif text-[clamp(2.5rem,4vw,3.8rem)] font-semibold leading-none tracking-[-.035em]">Subscribe to Access Your Library</h2><p className="mx-auto mt-6 max-w-[680px] text-[1.08rem] leading-8 text-[#756b61]">Saved cards and custom poetry requests are available to subscribers.<br/>Start your journey to unlock your personal poetry collection.</p>{error&&<p className="mt-4 text-sm text-red-700">{error}</p>}<button disabled={checkout} onClick={subscribe} className="mt-8 rounded-full bg-[#17392f] px-8 py-4 text-lg font-semibold text-white shadow-[0_12px_25px_rgba(23,57,47,.18)] disabled:opacity-60">{checkout?'Opening checkout…':'Subscribe Now'}</button></div></section>:<>
